...
d1.counts <- read.delim("~/Desktop/Research_projects_2015-2016/iCLIP/results_from_scripts/091417_Dbp2_meta-analysis_after_revised_filtering/091417_iCLIP_Dbp21_all_filtered_sites_in_bound_mRNAs_count_mat_66bins.txt", as.is = T)
d2.counts <- read.delim("~/Desktop/Research_projects_2015-2016/iCLIP/results_from_scripts/091417_Dbp2_meta-analysis_after_revised_filtering/091417_iCLIP_Dbp22_all_filtered_sites_in_bound_mRNAs_count_mat_66bins.txt", as.is = T)
d3.counts <- read.delim("~/Desktop/Research_projects_2015-2016/iCLIP/results_from_scripts/091417_Dbp2_meta-analysis_after_revised_filtering/091417_iCLIP_Dbp23_all_filtered_sites_in_bound_mRNAs_count_mat_66bins.txt", as.is = T)

##### Find out the "library size": the total count in each sample
sum(d1.counts) ### 280110
sum(d2.counts) ### 285749
sum(d3.counts) ### 297209

##### Divide the counts by "library size", per million
d1.counts.norm <- d1.counts/0.280110
d2.counts.norm <- d2.counts/0.285749
d3.counts.norm <- d3.counts/0.297209
...
tiff("091417_Dbp2_iCLIP_all_sites_metagene_in_bound_mRNA_prop_66bins_3rep_norm_toRPKM_res600.tiff", height = 4, width = 5, units = 'in', res = 600)
par(ps = 12, cex = 1, lwd = 2)
plot(0.5:65.5, apply(avg.prop.table, 2, mean, na.rm=T), cex.lab = 1.2, cex.axis = 1.1, type="l", lwd=2, col="blue3", yaxt="n", ylim = c(0, 0.045), xaxt = "n", xlab = "Relative transcript positions", ylab = "Dbp2 occupancy", main = "3 replicates averaged, Dbp2-binding mRNAs, normalized to RPKM")
