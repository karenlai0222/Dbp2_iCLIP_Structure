...
D23.all.sites <- read.delim("~/Desktop/Research_projects_2015-2016/iCLIP/results_from_scripts/010418_Data_from_Nadia/091317_iCLIP_Dbp23_intersected_reads_xlinking_sites_filtered.txt", quote = "", as.is = T)
D23.site.gr <- GRanges(seqnames = Rle(D23.all.sites$chr), ranges = IRanges(start = D23.all.sites$D3_xlinking_site, end = D23.all.sites$D3_xlinking_site), strand = Rle(D23.all.sites$strand), id = D23.all.sites$id, seqlengths = yeast.length)
...
# ## 5'UTR
# D23.utr5.prop.table.mod <- D23.utr5.prop.table
# utr5.nan <- is.nan(D23.utr5.prop.table.mod)
# D23.utr5.prop.table.mod[utr5.nan] <- 0
# ## ORF
# D23.orf.prop.table.mod <- D23.orf.prop.table
# orf.nan <- is.nan(D23.orf.prop.table.mod)
# D23.orf.prop.table.mod[orf.nan] <- 0
# ## 3'UTR
# D23.utr3.prop.table.mod <- D23.utr3.prop.table
# utr3.nan <- is.nan(D23.utr3.prop.table.mod)
# D23.utr3.prop.table.mod[utr3.nan] <- 0
...
# plot(apply(D23.utr5.prop.table.mod, 2, mean, na.rm=T), type="l", lwd=2, col="red", ylim=c(0, 0.5), main = "iCLIP_D23, 5'UTR")
# plot(apply(D23.utr5.prop.table, 2, median, na.rm=T), type="l", lwd=2, col="red", ylim=c(0, 0.5), main = "iCLIP_D23, 5'UTR")
...
write.table(D23.whole.mat, file = "~/Desktop/Research_projects_2015-2016/iCLIP/results_from_scripts/010418_Data_from_Nadia/010518_iCLIP_Dbp23_all_filtered_sites_in_bound_mRNAs_count_mat_100bins.txt", quote = F, sep = "\t")
write.table(D23.whole.prop.table, file = "~/Desktop/Research_projects_2015-2016/iCLIP/results_from_scripts/010418_Data_from_Nadia/010518_iCLIP_Dbp23_all_filtered_sites_in_bound_mRNA_prop_table_100bins.txt", quote = F, sep = "\t")
...
par(lwd = 2)
plot(0.5:99.5, apply(D23.whole.prop.table, 2, mean, na.rm=T), cex.lab = 1.2, cex.axis = 1.1, type="l", lwd=4, col="blue3", yaxt="n", ylim = c(0, 0.03), xaxt = "n", xlab = "Relative transcript positions", ylab = "Dbp2 occupancy", main = "iCLIP_D23, Dbp2-binding mRNAs, all sites")
...
## Make the dataframes (countoverlaps) for iCLIP Dbp2-1, 5'UTR
D23.utr5.pos.count.mat <- matrix(nrow=length(utr5.pos.gr), ncol=5)
colnames(D23.utr5.pos.count.mat) <- 1:5
rownames(D23.utr5.pos.count.mat) <- mcols(utr5.pos.gr)$id
for(i in 1:5){
    temp.shift <- shift(utr5.pos.gr, shift=utr5.shift.mat.pos[,i])
    temp.count <- countOverlaps(temp.shift, D23.site.gr)
...
D23.utr5.neg.count.mat <- matrix(nrow=length(utr5.neg.gr), ncol=5)
colnames(D23.utr5.neg.count.mat) <- 1:5
rownames(D23.utr5.neg.count.mat) <- mcols(utr5.neg.gr)$id
for(i in 1:5){
    temp.shift <- shift(utr5.neg.gr, shift= -utr5.shift.mat.neg[,i])
    temp.count <- countOverlaps(temp.shift, D23.site.gr)
...
## Make the dataframes (countoverlaps) for iCLIP Dbp2-1, ORF
D23.orf.pos.count.mat <- matrix(nrow=length(orf.pos.gr), ncol=55)
colnames(D23.orf.pos.count.mat) <- 1:55
for(i in 1:55){
    temp.shift <- shift(orf.pos.gr, shift=orf.shift.mat.pos[,i])
    temp.count <- countOverlaps(temp.shift, D23.site.gr)
    D23.orf.pos.count.mat[, i] <- temp.count
...
D23.orf.neg.count.mat <- matrix(nrow=length(orf.neg.gr), ncol=55)
colnames(D23.orf.neg.count.mat) <- 1:55
for(i in 1:55){
    temp.shift <- shift(orf.neg.gr, shift= -orf.shift.mat.neg[,i])
    temp.count <- countOverlaps(temp.shift, D23.site.gr)
    D23.orf.neg.count.mat[, i] <- temp.count
...
## Make the dataframes (countoverlaps) for iCLIP Dbp2-1, 3'UTR
D23.utr3.pos.count.mat <- matrix(nrow=length(utr3.pos.gr), ncol=6)
colnames(D23.utr3.pos.count.mat) <- 1:6
for(i in 1:6){
    temp.shift <- shift(utr3.pos.gr, shift=utr3.shift.mat.pos[,i])
    temp.count <- countOverlaps(temp.shift, D23.site.gr)
    D23.utr3.pos.count.mat[, i] <- temp.count
...
D23.utr3.neg.count.mat <- matrix(nrow=length(utr3.neg.gr), ncol=6)
colnames(D23.utr3.neg.count.mat) <- 1:6
for(i in 1:6){
    temp.shift <- shift(utr3.neg.gr, shift= -utr3.shift.mat.neg[,i])
    temp.count <- countOverlaps(temp.shift, D23.site.gr)
    D23.utr3.neg.count.mat[, i] <- temp.count
...
D23.whole.prop.table <- prop.table(D23.whole.mat, 1)
### Output the count table and the probability table
write.table(D23.whole.mat, file = "~/Desktop/Research_projects_2015-2016/iCLIP/results_from_scripts/091417_Dbp2_meta-analysis_after_revised_filtering/091417_iCLIP_Dbp23_all_filtered_sites_in_bound_mRNAs_count_mat_66bins.txt", quote = F, sep = "\t")
write.table(D23.whole.prop.table, file = "~/Desktop/Research_projects_2015-2016/iCLIP/results_from_scripts/091417_Dbp2_meta-analysis_after_revised_filtering/091417_iCLIP_Dbp23_all_filtered_sites_in_bound_mRNA_prop_table_66bins", quote = F, sep = "\t")
### Plot the whole transcript
setwd("~/Desktop/Research_projects_2015-2016/iCLIP/results_from_scripts/091417_Dbp2_meta-analysis_after_revised_filtering/")
tiff("091417_Dbp23_iCLIP_all_sites_metagene_in_bound_mRNA_prop_66bins_res600.tiff", height = 4, width = 5, units = 'in', res = 600)
par(lwd = 2)
plot(0.5:65.5, apply(D23.whole.prop.table, 2, mean, na.rm=T), cex.lab = 1.2, cex.axis = 1.1, type="l", lwd=4, col="blue3", yaxt="n", ylim = c(0, 0.04), xaxt = "n", xlab = "Relative transcript positions", ylab = "Dbp2 occupancy", main = "iCLIP_D23, Dbp2-binding mRNAs, all sites")
