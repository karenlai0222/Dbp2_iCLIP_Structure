...
D22.all.sites <- read.delim("Dbp2iCLIP_sitesFromReproducedReads/060618_iCLIP_Dbp22_intersectedReads_allxlinkingSites.txt",
                            quote = "", as.is = T)
D22.site.gr <- GRanges(seqnames = Rle(D22.all.sites$chr),
                       ranges = IRanges(start = D22.all.sites$D2_xlinking_site, end = D22.all.sites$D2_xlinking_site),
                       strand = Rle(D22.all.sites$strand), id = D22.all.sites$id,
...
utr5.neg.gr <- GRanges(seqnames = Rle(dbp2.mRNA.info.neg$chr),
                       ...
                       seqlengths = yeast.length)  
## Make the dataframes (countoverlaps) for iCLIP Dbp2-2, 5'UTR
...
write.table(D22.whole.mat, file = "060718_iCLIP_Dbp22_FilteredSites_inBoundmRNAs_countMat100bins.txt",
            quote = F, sep = "\t")
write.table(D22.whole.prop.table,
            file = "060718_iCLIP_Dbp22_all_FilteredSites_inBoundmRNA_propTable100bins.txt",
            quote = F, sep = "\t")
### Plot the whole transcript (Do this on my computer)
D22.whole.prop.table <- read.delim("~/Desktop/Research_projects_2015-2016/iCLIP/results_from_scripts/060618_Dbp2iCLIPdata_reprocessing/060718_iCLIP_Dbp22_all_FilteredSites_inBoundmRNA_propTable100bins.txt",
                                   as.is = T)
...
par(lwd = 2)
plot(0.5:99.5, apply(D22.whole.prop.table, 2, mean, na.rm=T), cex.lab = 1.2, cex.axis = 1.1,
     type="l", lwd=4, col="blue3", yaxt="n", ylim = c(0, 0.03), xaxt = "n",
     xlab = "Relative transcript positions", ylab = "Dbp2 occupancy",
     main = "iCLIP_D22, Dbp2-binding mRNAs, all sites")
